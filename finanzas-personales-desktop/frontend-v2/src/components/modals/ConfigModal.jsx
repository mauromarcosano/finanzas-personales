...
export default function ConfigModal({ isOpen, onClose, categorias, setCategorias }) {
  ...
  const safeCatsProp = Array.isArray(categorias) ? categorias : [];
  const [localCats, setLocalCats] = useState(() => JSON.parse(JSON.stringify(safeCatsProp)));
  const [selectedCatId, setSelectedCatId] = useState(() => (safeCatsProp.length > 0 ? safeCatsProp[0].id : null));
