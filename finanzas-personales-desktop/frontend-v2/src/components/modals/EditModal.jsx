...
const EditModal = ({ editingGasto, setEditingGasto, handleSaveEdit, handleDelete, categoriasConfig }) => {
  const safeCats = Array.isArray(categoriasConfig) ? categoriasConfig : [];
  const [selectedCat, setSelectedCat] = React.useState(editingGasto.categoria || (safeCats?.[0]?.id) || 'Otros');
  const onDeleteClick = () => {
          ...
          <div className="form-grid-2">
            ...
            <div className="form-group">
              ...
              <select name="subcategoria" required defaultValue={editingGasto.subcategoria || 'Otros'}>
                {(() => {
                  const catData = safeCats.find(c => c.id === selectedCat);
                  if (!catData || !catData.subcategorias || !catData.subcategorias.length) {
